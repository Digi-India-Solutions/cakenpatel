...
//         setIsLoading(true);
//         try {
//             const res = await axios.get(`${BASE_URL}/api/franchise/enquiry/all`, {
...
//             }
//         } catch (err) {
//             toast.error("Failed to fetch franchise enquiries!");
//             console.error(err);
//         } finally {
//             setIsLoading(false);
//         }
...
//                 `${BASE_URL}/api/franchise/enquiry/update-status/${id}`,
//                 { status: newStatus }
//             );
//             if (res.data.success) {
...
//             try {
//                 await axios.delete(`${BASE_URL}/api/franchise/enquiry/delete/${id}`);
...
//                                     {/* Name */}
//                                     <td style={{ fontWeight: "500", whiteSpace: "nowrap" }}>{enq.name}</td>
...
// export default AllFranchiseEnquiry;
        ...
        >
            ...
            >
                ...
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "20px" }}>
                    <h5 style={{ margin: 0, fontWeight: 600, color: "#1a1a2e" }}>Franchise Enquiry Details</h5>
                    <button
                        onClick={onClose}
                ...
                {[
                    ...
                    <div key={label} style={{ display: "flex", gap: "12px", marginBottom: "12px", fontSize: "14px" }}>
                        <span style={{ minWidth: "140px", color: "#777", fontWeight: 500 }}>{label}</span>
...
const AllFranchiseEnquiry = () => {
    ...
    // ── Fetch ─────────────────────────────────────────────────────────────────
    const fetchEnquiries = useCallback(async (page = 1, search = "", status = "") => {
        setIsLoading(true);
        try {
            const res = await axios.get(`${BASE_URL}/api/franchise/enquiry/all`, {
                params: { page, limit: LIMIT, search, status },
                ...
                <table className="table table-bordered table-striped table-hover">
                    ...
                    <tbody>
                        {isLoading ? (
                            ...
                            <tr>
                                <td colSpan="11" className="text-center py-4" style={{ color: "#999" }}>
                                    No franchise enquiries found.
                                </td>
                            </tr>
                        ) : (
                            enquiries.map((enq, index) => (
                                <tr key={enq._id}>
                                    ...
                                    <td style={{ whiteSpace: "nowrap" }}>
                                        {enq.phone
                                            ? <a href={`tel:${enq.phone}`} style={{ color: "#2e6a7c", textDecoration: "none", fontWeight: "500" }}>{enq.phone}</a>
                                            : "—"
                                        }
                                    </td>
                                    ...
                                    <td style={{ whiteSpace: "nowrap" }}>
                                        <button
                                            ...
                                            className="bt"
                                            style={{
                                                marginRight: "6px", background: "#e3f2fd",
